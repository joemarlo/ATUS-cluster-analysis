...
```{r include=FALSE}
# memory management -------------------------------------------------------
rm(atus_wide, corr_matrix, pca_plot, total_rows)
gc()
```
...
# full tree with labels colored by cluster
dend_data <- dendro_data(dend, type = "rectangle")
...
# plot full tree with labels colored
ggplot(dend_data$segments) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend)) +
...
dend_data <- dendro_data(dend_cut, type = "rectangle")
...
dend_data$segments$cluster <- as.factor(cluster_membership)
linetypes <- c('solid', 'dashed')[as.numeric(is.na(dend_data$segments$cluster))+1]
